...
#2. Plot3() - use ExA_read() function to load and prepare the data, and creates ".png" file 
...
#4. call Plot3() function
#5. find output "plot3.png" file in R Working Directory
...
Plot3 <- function () {
        ...
        plot(                                                           #create a plot with one line
                df$Time,df$Sub_metering_1, type = "l", xlab = "", 
                ylab = "Energy sub metering"
        )
        lines(df$Time,df$Sub_metering_2, col = "red")                   #add second line to a plot
        lines(df$Time,df$Sub_metering_3, col = "blue")                  #add third line to a plot
        
        legend(                                                         #add plot legend
                "topright", col = c("black", "red", "blue"),
                lty = c(1,1,1),legend = c("Sub_metering_1", 
                                          "Sub_metering_2", 
                                          "Sub_metering_3")
        )
